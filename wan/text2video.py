...
from .distributed.cfg_parallel import (
    cfg_all_gather,
    get_cfg_size,
    is_cond_rank,
)
        ...
        with (
            ...
            with profiled_loop() as loop:
              for step_idx, t in enumerate(tqdm(timesteps)):
                with loop.step(step_idx) as spans:
                    ...
                    if get_cfg_size() > 1:
                        with spans.span("model_forward_cfg_split"):
                            local_arg = arg_c if is_cond_rank() else arg_null
                            local_pred = model(
                                latent_model_input, t=timestep, **local_arg)[0]
                        with spans.span("cfg_all_gather"):
                            noise_pred_cond, noise_pred_uncond = cfg_all_gather(
                                local_pred)
                    else:
            ...
            if offload_model:
                with trace_span("dit_free"):
                    del self.low_noise_model, self.high_noise_model
                    gc.collect()
                    torch.cuda.empty_cache()
            if self.rank == 0:
