...
4. **Quality — visual A/B vs Phase 2 baseline.** torch.compile composition with fp8+sage must not drift further from gold than sage-vs-flash already does. Per-frame PSNR vs Phase 2 mp4 is **mean PSNR >= 35 dB on motion-heavy prompts**. (Recalibrated 2026-04-30 from initial 40 dB target — see "Why 35 dB and not 40" below.) Static-prompt comparisons (cat-meadow) are documented as non-gating because rollout bifurcation produces uniformly-low PSNR across all phase-on-phase comparisons even when output is qualitatively fine; the cat-meadow signal is consistent with motion-heavy result and visual review.
...
- **Phase-on-phase tolerance is tighter** because sage-vs-flash and compile-vs-eager are supposed to be near-identical in precision, NOT lossy. PSNR 34/35 dB respectively are sanity checks that the integration didn't subtly break. (Phase 3 originally specified 40 dB; recalibrated to 35 dB on 2026-04-30 after measurement — see "Why 35 dB and not 40" below.)
...
**Merged to demo-server (2026-04-30).** After visual verification of Phase 3 mp4s (jellyfish near-identical to Phase 2; cat shows the same rollout-bifurcation pattern Phase 2 already had), the strict 40 dB gate was recalibrated to 35 dB and Phase 3 was merged via fast-forward. Rationale captured in "Why 35 dB and not 40" below.

### Why 35 dB and not 40

The original 40 dB Phase-3-vs-Phase-2 threshold assumed compile-vs-eager would be numerically near-identical the way it is under bf16 alone. Stacked on top of fp8 + sage, that assumption is wrong. Three observations forced the recalibration:

1. **Phase 3 vs bf16 jellyfish (34.72 dB) is BETTER than Phase 2 vs bf16 (34.36 dB).** Compile is not drifting away from the gold reference; it is rolling slightly toward it. The "drift" measured by Phase-3-vs-Phase-2 is a one-shot kernel-reorder offset, not cumulative noise. Absolute distance to bf16 is the real quality floor — and it stayed flat or improved.
2. **Bit-exact across-request determinism within Phase 3** (gen 2 == gen 3 == gen 4, PSNR=inf) confirms the compiled graph itself is deterministic. The phase-on-phase difference is a fixed kernel-reorder, not a sampler timing race.
3. **Each phase introduces ~37 dB drift to its predecessor under fp8 stacking** — Phase 2 vs Phase 1 jellyfish was 36.60 dB; Phase 3 vs Phase 2 jellyfish was 36.80 dB. Sage and compile are roughly co-equal sources of rounding noise. The 40 dB floor was simply too tight for stacked fp8+sage+compile composition.

The 35 dB threshold preserves the regression-detection intent (a graph-break or kernel-fusion bug would drop PSNR much further than this) while accepting the realistic noise floor of the optimization stack. Future A/B gates for any layer added on top of this stack should expect a similar ~36-37 dB phase-on-phase number against this Phase 3 baseline, NOT against bf16.
