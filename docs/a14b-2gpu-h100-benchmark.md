...
For **maximum latency**, accepting the diffusers code path as the production reference: **SGLang S1** at 393 s — 2.10× faster than Y4, 2.71× faster than vanilla diffusers itself. Output is **16.5 dB from vanilla diffusers and 17.2 dB from in-tree Y4** — both within the diffusers-RNG-path cluster, no evidence of additional lossy approximation beyond what diffusers itself does relative to in-tree. Decision is: do you bind your serving reference to `generate.py` (then only Y4 is bit-equal) or to `diffusers.WanPipeline` (then SGLang gives you 2.71× over the obvious diffusers baseline at no extra quality cost)?
